...
<?php
		...
		if(isset($_POST['deleteC']))
		{
			...
			$res=mysql_query( "DELETE FROM voters  WHERE `electbody`='$electbody' AND `email`='$email'");
			if($res)
			{
				echo ("<SCRIPT LANGUAGE='JavaScript'>window.alert('Voter successfully deleted..')
                window.location.href='';</SCRIPT>");		
			}
			else
			{
				echo ("<SCRIPT LANGUAGE='JavaScript'>window.alert('The system was unable to delete the Voter..')
                window.location.href='';</SCRIPT>");
			}
		}
			...
			<div class="col-md-6 login-do animated wow fadeInCenter" data-wow-delay=".5s">
				...
				<div class="login-mail">
					<?php 
							...
							echo"  <option selected disabled hidden>Select Voter Email</option>";
							echo"  <option>";
								require('../inc/connect.inc.php');//connects to database
								$query = "SELECT * FROM voters WHERE `electbody`='$electbody'";//selects all our clients
					...
					<div class=" login-do animated wow fadeInRight" data-wow-delay=".5s" align="center">
						<label class="hvr-sweep-to-top login-sub">
							<input type="submit" value="Delete Voter" name='deleteC'>
							</label>
					</div>
