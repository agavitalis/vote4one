...
?>
	...
	$strQuery= "SELECT * FROM contestants WHERE `electbody`='$ahaulo' AND `post`='Treasurer'";
		//$row=mysql_fetch_array($Total);
     	...
     	if ($result) {
        	...
        	$arrData = array(
                "chart" => array(
                    "caption" => "Tresurer Election Results",
	...
	// Push the data into the array
			...
			$v = $row["fname"];
			if($vita=mysqli_query( $dbhandle,"SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Treasurer' AND `candidateVoted`='$v'"))
		 ...
		 $totalVotecast = "SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Treasurer' ";
...
<div class="col-md-9 animated wow fadeInRight" data-wow-delay=".5s"><!--//General div fades in rigbt-->	
				...
				<div class="mens-toolbar">
					<div class="colors animated wow fadeInUp animated" data-wow-delay=".5s" >
					<h3 id= "msg"><?php if(!$vita){echo "NO ELECTIONS RESULTS YET FOR THE TREASURER";}
					else{echo "TREASURER ELECTIONS RESULTS"; }?>
					</div>
	    		    <div class="clearfix"></div>		
		        </div>
...
<div class="mid-popular">
  ...
  <div class="container" >	
		...
		<p><h3><center><u>Summary of the Treasurer's Elections</u></center></h3></br>
	...
	// Establish a connection to the database
	  ...
	  // Render an error message, to avoid abrupt failure, if the database connection parameters are incorrect
	   if ($dbhandle->connect_error) {
		...
		$str = "SELECT * FROM contestants WHERE `electbody`='$ahaulo' AND`post`='Treasurer'";
		...
		while($rorr = mysqli_fetch_array($t)) {
			$f = $rorr["fname"];
			$l = $rorr["lname"];
			$diebere = mysqli_query( $dbhandle,"SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Treasurer' AND `candidateVoted`='$f'");
