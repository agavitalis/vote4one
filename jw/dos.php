...
?>
 	<?php
		...
		$strQuery= "SELECT * FROM contestants WHERE  `electbody`='$ahaulo' AND `post`='Director_Of_Social'";
		//$row=mysql_fetch_array($Total);
     	...
     	if ($result) {
        	...
        	$arrData = array(
                "chart" => array(
                    "caption" => "Director of Social Election Result",
	...
	// Push the data into the array
			...
			$v = $row["fname"];
			if($vita=mysqli_query( $dbhandle,"SELECT * FROM voting WHERE `electbody`='$ahaulo' AND`postVotedFor`='Director_Of_Social' AND `candidateVoted`='$v'"))
        	...
        	$columnChart->render();
			 ...
			 $totalVotecast = "SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Director_Of_Social' ";
...
<div class="col-md-9 animated wow fadeInRight" data-wow-delay=".5s"><!--//General div fades in rigbt-->	
				...
				<div class="mens-toolbar">
					<div class="colors animated wow fadeInUp animated" data-wow-delay=".5s" >
					<h3 id= "msg"><?php if(!$vita){echo "NO ELECTIONS RESULTS YET FOR THE DIRECTOR OF SOCIAL";}
					else{echo "DIRECTOR OF SOCIAL ELECTIONS RESULTS"; }?>
					</div>
	    		    <div class="clearfix"></div>		
		        </div>
...
<div class="mid-popular">
	...
	<div class="container" >	
		...
		<p><h3><center><u>Summary of the Director-Of-Social Elections</u></center></h3></br>
	...
	// Establish a connection to the database
	  ...
	  // Render an error message, to avoid abrupt failure, if the database connection parameters are incorrect
	   if ($dbhandle->connect_error) {
		...
		$str = "SELECT * FROM contestants WHERE `electbody`='$ahaulo' AND`post`='Director_Of_Social'";
		...
		while($rorr = mysqli_fetch_array($t)) {
			$f = $rorr["fname"];
			$l = $rorr["lname"];
			$diebere = mysqli_query( $dbhandle,"SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Director_Of_Social' AND `candidateVoted`='$f'");
...
</div><!--Director of Social Div  ends-->
