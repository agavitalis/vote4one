...
?>
 	<?php
		...
		$strQuery= "SELECT * FROM contestants WHERE `electbody`='$ahaulo' AND `post`='Secretery'";
		//$row=mysql_fetch_array($Total);
     	...
     	if ($result) {
        	...
        	$arrData = array(
                "chart" => array(
                    "caption" => "Secretery General Election Result",
	...
	// Push the data into the array
			...
			$v = $row["fname"];
			if($vita=mysqli_query( $dbhandle,"SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Secretery' AND `candidateVoted`='$v'"))
        	...
        	$columnChart->render();
			 ...
			 $totalVotecast = "SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Secretery' ";
...
<div class="col-md-9 animated wow fadeInRight" data-wow-delay=".5s"><!--//General div fades in rigbt-->	
				...
				<div class="mens-toolbar">
					<div class="colors animated wow fadeInUp animated" data-wow-delay=".5s" >
					<h3 id= "msg"><?php if(!$vita){echo "NO ELECTIONS RESULTS YET FOR THE SECRETERY";}
					else{echo " SECRETERY GENERAL ELECTIONS RESULTS"; }?>
					</div>
	    		    <div class="clearfix"></div>		
		        </div>
...
<div class="mid-popular">
  ...
  <div class="container" >	
		...
		<p><h3><center><u>Summary of the Secretery Elections</u></center></h3></br>
	...
	$str = "SELECT * FROM contestants WHERE `electbody`='$ahaulo' AND`post`='Secretery'";
		...
		while($rorr = mysqli_fetch_array($t)) {
			$f = $rorr["fname"];
			$l = $rorr["lname"];
			$diebere = mysqli_query( $dbhandle,"SELECT * FROM voting WHERE `electbody`='$ahaulo' AND `postVotedFor`='Secretery' AND `candidateVoted`='$f'");
